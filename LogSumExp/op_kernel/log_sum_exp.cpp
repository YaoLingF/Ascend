...
public:
    ...
    __aicore__ inline void Init(GM_ADDR x, GM_ADDR y, int32_t *shape,int32_t ts)
    {
            ...
            else if(ts==2) yGm.SetGlobalBuffer((__gm__ TYPE_X *)y, shape[1]*shape[3]);
            else if(ts==3) yGm.SetGlobalBuffer((__gm__ TYPE_X *)y, shape[1]*shape[2]);
                    ...
                    for(int i=0;i<shape[1];i++)
                    {
                        ...
                        for(int j=0;j<shape[3];j++)
                        {
                            ...
                            for(int k=0;k<shape[0];k++)
                            {
                                ...
                                {
                                    LocalTensor<TYPE_X> x1 = X.AllocTensor<TYPE_X>();
                                    DataCopy(x1, xGm[k*shape[1]*shape[2]*shape[3]+i*shape[2]*shape[3]+z*shape[3]+j], 32);
                                    X.EnQue(x1);
                                    LocalTensor<TYPE_X> x2 = X.DeQue<TYPE_X>();
                                    Exp(x2,x2,32);
                            ...
                            LocalTensor<float> y2 = Y.DeQue<float>();
                            Ln(y2,y2,32);
                            yGm.SetValue(i*shape[3]+j,y2.GetValue(0));
                            Y.FreeTensor(y2);

                            //yGm.SetValue(i*shape[3]+j,float(0.0));
                ...
                else if(ts==3)
                {
                    ...
                    for(int i=0;i<shape[1];i++)
                    {
                        for(int j=0;j<shape[2];j++)
                        {
                            ...
                            // {
                            //     LocalTensor<TYPE_X> x1 = X.AllocTensor<TYPE_X>();
                            //     DataCopy(x1, xGm[k*shape[1]*shape[2]+i*shape[2]+j], 32);
                            //     X.EnQue(x1);
                            //     LocalTensor<TYPE_X> x2 = X.DeQue<TYPE_X>();
                            //     Exp(x2,x2,32);
                            //     float val = x2.GetValue(0); 
                            ...
                            yGm.SetValue(i*shape[2]+j,float(0.0));
